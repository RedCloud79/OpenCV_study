...
video = pafy.new(url)
# 제목, 평점, 조회수, 저작권자, 길이, 길이, 좋아요, 싫어요 의 정보를 가지고 있다.

cap = cv2.VideoCapture(0)
# print('cap.isOpend:', cap.isOpened)

if cap.isOpened():
    while True:
        ret, frame = cap.read()
        if not ret: break # 해당 프레임이 유효한지 파악

        # 유효한 프레임은 윈도우에 표시
        cv2.imshow('YOUTUBE', frame)

        # 프레임 간에 딜레이를 준다. (화면에 유지될 시간)
        # 만약 아무 키나 입력을 한다면 영상 플레이를 중단 및 종료
...
cap.release()
cv2.destroyAllWindows()
# cv2.destroyWindow('YOUTUBE') # 특정 이름의 윈도우를 종료
```
* 에러남 해결 대기
------------------------------------------------------------
